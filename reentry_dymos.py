...
phase0.set_state_options("h", fix_initial=True, fix_final=True, units="ft", rate_source="hdot", targets=["h"], lower=0)
...
phase0.set_state_options("v", fix_initial=True, fix_final=True, units="ft/s", rate_source="vdot", targets=["v"], lower=0)
...
phase0.add_path_constraint("q", lower=0, upper=70, units="Btu/ft**2/s")
...
#Note:current jacobian in total_jac_info.pickle is from a run_model case and 
#cannot be used for PJRNScaler (refs that lead to convergence can be found on
# master branch and can be implemented to get jacobian of fully converged model)

# autoscaler = None
autoscaler = IsoScaler(J, lower_bounds, upper_bounds)
# autoscaler = PJRNScaler(J, lower_bounds, upper_bounds)
